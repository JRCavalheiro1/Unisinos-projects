...
//EXERCICIO DA ETAPA 3 DO DESAFIO SE ENCONTRA NA LINHA 104
public class StaticList<E> implements List<E> {
	...
	public int contaElementos(E el) throws IllegalArgumentException{
		if(this.isFull()) throw new IllegalArgumentException(); 
		return contaElementos(el, 0, 0);
	}

	private int contaElementos( E el, int i, int qtdElementos) {
		if(i > numElements-1) return qtdElementos;
		if(this.get(i) == el){
			qtdElementos++;
			i++;
			return contaElementos(el,i, qtdElementos);
		}
		i++;
		return contaElementos(el,i, qtdElementos);
	}
